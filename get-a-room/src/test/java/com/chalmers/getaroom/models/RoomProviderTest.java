...
class RoomProviderTest {
    ...
    @Test
    void getNewDataToCache_innerExceptionLogs() throws IOException {
        ChalmersMapsAPIInterface api = mock(ChalmersMapsAPIInterface.class);
        ...
        when(api.getTimeEditInfo(any())).thenThrow(new IOException("Test exception"));
        ...
        try {
            List<Room> rooms = roomProvider.getNewDataToCache();
            assertEquals(1, rooms.size());
            assertEquals(-1, rooms.get(0).seats());
        } catch (IOException e) {
            fail(e);
        }
    }
    ...
    @Test
    void getNewDataToCache_outerExceptionsThrows() throws IOException {
        ChalmersMapsAPIInterface api = mock(ChalmersMapsAPI.class);
        when(api.informationBoard(any())).thenThrow(new IOException("Test exception"));

        RoomProvider provider = new RoomProvider(api);

        assertThrows(IOException.class, provider::getNewDataToCache);
    }
}